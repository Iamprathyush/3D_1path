...
export default function TunnelSection() {
  ...
  // Section 1: 0% to 33%
  const s1Scale = useTransform(smoothProgress, [0.0, 0.15, 0.25, 0.33], [0, 1, 1, 4]);
  const s1Opacity = useTransform(smoothProgress, [0.0, 0.1, 0.25, 0.33], [0, 1, 1, 0]);
  
  // Section 2: 33% to 66%
  const s2Scale = useTransform(smoothProgress, [0.33, 0.48, 0.58, 0.66], [0, 1, 1, 4]);
  const s2Opacity = useTransform(smoothProgress, [0.33, 0.4, 0.58, 0.66], [0, 1, 1, 0]);
  
  // Section 3: 66% to 100%
  const s3Scale = useTransform(smoothProgress, [0.66, 0.85], [0, 1]);
  const s3Opacity = useTransform(smoothProgress, [0.66, 0.75], [0, 1]);
...
`;
    ...
    const tunnelUniforms = {
      uTime:       { value: 0 },
      uAppear:     { value: 0 },
      uColLow:     { value: hexToVec3('#111111') }, // decent black
      uColHigh:    { value: hexToVec3('#ffffff') }, // white
      uOpacity:    { value: 1.44 },
      uSize:       { value: 5 },
...
`
    ...
    tunnelMesh.layers.set(LAYERS.ENTIRE_SCENE);
    ...
    const atmoMat = new THREE.ShaderMaterial({
      ...
      uniforms: {
        uTime: { value: 0 },
        uColor: { value: hexToVec3('#999999') }, // Gray motes
...
`
    ...
    const atmoMesh = new THREE.Points(atmoGeo, atmoMat);
    atmoMesh.frustumCulled = false;
    atmoMesh.layers.set(LAYERS.ENTIRE_SCENE);
    scene.add(atmoMesh);
    ...
    const FinalPass = {
      uniforms: {
        ...
        uBg: { value: hexToVec3('#000000') }, // black bg
        uFlameA: { value: hexToVec3('#444444') }, // gray flame
        uFlameB: { value: hexToVec3('#888888') }, // light gray flame
        uFlameAmt: { value: 0.2 }
      },
...
`
    ...
    const onScroll = () => {
      const sh = containerRef.current ? containerRef.current.offsetHeight : window.innerHeight * 3;
      
      // Calculate scroll progress through this component
      const rect = containerRef.current.getBoundingClientRect();
      // rect.top is 0 when the container is at the top of the viewport
      const scrollY = -rect.top; 
      scrollTarget = clamp(scrollY / (sh - window.innerHeight), 0, 1);
      
      if (canvasRef.current) {
        // Fade out the canvas to black during the last 30% of the scroll
        const fade = clamp((1 - scrollTarget) / 0.3, 0, 1);
    ...
    // ---------- Loop ----------
    const sceneObj = { t0: performance.now() / 1000, rollPhase: 0, appearStart: performance.now() };
    sceneObj.render = (scroll, m) => {
      const t = performance.now() / 1000;
      const dt = Math.min(0.05, t - sceneObj.t0); sceneObj.t0 = t;
      ...
      tunnelUniforms.uSwirl.value = 0.39 * (1 + scroll * 1.5);
      sceneObj.rollPhase += dt * (0.065 + scroll * 0.05);
      tunnelGroup.rotation.z = sceneObj.rollPhase;
      ...
      atmoMesh.position.copy(camera.position);
  ...
  return (
    <div ref={containerRef} id="scroll-host" className="relative w-full bg-black" style={{ height: '600vh' }}>
      <canvas
        id="scene"
        ref={canvasRef}
        className="sticky top-0 left-0 w-full h-screen block pointer-events-none"
        style={{ zIndex: 0 }}
      />
      
      {/* Scroll Hint */}
      <div id="scroll-hint" className="fixed left-0 right-0 bottom-[18px] text-center text-white/55 text-[11px] uppercase tracking-[0.15em] pointer-events-none z-10 font-sans">
        scroll ↓
      </div>

      {/* Overlay content container with a subtle radial gradient to improve contrast */}
      <div className="sticky top-0 left-0 w-full h-screen z-10 pointer-events-none overflow-hidden ">
        
        {/* SECTION 1 */}
        <motion.div 
          style={{ scale: s1Scale, opacity: s1Opacity }}
          className="absolute inset-0 flex flex-col items-center justify-center p-5 text-center drop-shadow-2xl"
        >
          <motion.h1 
            className="text-white font-zentry font-black text-5xl md:text-7xl uppercase whitespace-nowrap mb-2 tracking-widest"
            style={{ textShadow: "0px 4px 20px rgba(0,0,0,0.9)" }}
          >
           From Overlooked to Unforgettable
          </motion.h1>
          <h2 
            className="special-font font-zentry font-black text-3xl md:text-5xl text-white uppercase"
            style={{ textShadow: "0px 4px 15px rgba(0,0,0,0.9)" }}
          >
           Let’s Build Your Visual Campaign.
          </h2>
        </motion.div>
        ...
        <motion.div 
          ...
          className="absolute inset-0 flex flex-col items-center justify-center p-5 text-center pointer-events-auto drop-shadow-2xl"
        >
          <motion.h1 
            className="text-white font-zentry font-black text-5xl md:text-7xl uppercase whitespace-nowrap mb-2 tracking-widest"
            style={{ textShadow: "0px 4px 20px rgba(0,0,0,0.9)" }}
          >
            Get Seen, Trusted, and Remembered.
          </motion.h1>
          <h2 
            className="special-font font-zentry font-black text-3xl md:text-5xl text-gray-300 mb-10 uppercase"
            style={{ textShadow: "0px 4px 15px rgba(0,0,0,0.9)" }}
          >
            Start Your Visual Campaign Today.
          </h2>
          <Button 
            id="enter-void" 
            title="Start Now" 
            leftIcon={<TiLocationArrow />} 
            containerClass="bg-white flex-center gap-1 pointer-events-auto shadow-xl" 
          />
        </motion.div>
